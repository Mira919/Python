...
admin.py (добавление админки)

import_phones.py  (читаем csv файл, берем данные и создаем их них новые модели(записи в БД))

models.py (создаем модель и поля для БД)

views.py (основные функции (сортировка по цене и тд))

catalog.html (шаблон HTML по сортировке)

product.html (шаблон HTML по информации о телефоне)
## Задание
