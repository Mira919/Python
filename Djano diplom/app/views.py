...
def add_to_cart_view(request, product_id): # функция показа корзины
    if request.user.is_authenticated:
        ...
        try:
            ...
            product_order_relation = OrderProductRelation.objects.create(order=actual_order, product=product, amount=0,
                                                                         total=0)
...
def show_cart_view(request):
    ...
    if request.user.is_authenticated:
        this_user = request.user
        try:
            actual_order = Order.objects.prefetch_related('products')\
                .only('products__name', 'products__description', 'products__price')\
                .get(customer=this_user, complete=False)
        except ObjectDoesNotExist:
            actual_order = Order.objects.prefetch_related('products')\
                .only('products__name', 'products__description', 'products__price')\
                .create(customer=this_user, total=0, complete=False)
        if request.method == 'POST':
